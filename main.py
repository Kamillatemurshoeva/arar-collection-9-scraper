...
START_URL = "https://arar.sci.am/dlibra/results?q=&action=SimpleSearchAction&type=-6&p=0&qf1=collections:9"
LIST_URL = "https://arar.sci.am/dlibra/results?q=&action=SimpleSearchAction&type=-6&p={}&qf1=collections:9"
...
def parse_item(session, url):
    ...
    other_title = pick_field(fields, ["Other title", "Alternative title", "Այլ վերնագիր", "Другое заглавие"])
    remainder_title = pick_field(fields, ["Remainder of title"])
    ...
    description = pick_field(
        fields,
        ["Abstract", "Description", "Ամփոփում", "Описание", "Table of contents", "Բովանդակություն", "Содержание"]
    )
    if not description:
    ...
    journal_title = pick_field(fields, ["Journal or Publication Title"])
    ...
    file_type = pick_field(fields, ["File type", "Mime type", "MIME type"])
    ...
    volume = pick_field(fields, ["Volume", "Հատոր", "Том"])
    number = pick_field(fields, ["Number", "Համար", "Номер"])
    isbn = pick_field(fields, ["ISBN"])
    issn = pick_field(fields, ["ISSN"])
    ...
    subjects = pick_field(
        fields,
        ["Subject and keywords", "Subjects", "Subject", "Keywords", "Թեմա և հիմնաբառեր", "Ключевые слова"]
    )
    official_url = pick_field(fields, ["Official URL"])
    ...
    for a in soup.find_all("a", href=True):
        ...
        low = href.lower()
        if not media_url and (low.endswith(".mp4") or low.endswith(".pdf") or low.endswith(".mp3") or low.endswith(".wav")):
            media_url = href
            break
    ...
    record = {
        ...
        "remainder_of_title": remainder_title,
        ...
        "year": year,
        "type": type_value,
        "journal_or_publication_title": journal_title,
...
def main():
    ...
    for i, url in enumerate(tqdm(links, desc="Scraping"), start=1):
        ...
        if i % 25 == 0:
            save_csv(records, "arar_collection_9_partial.csv")
            save_jsonl(records, "arar_collection_9_partial.jsonl")

    save_csv(records, "arar_collection_9.csv")
    save_jsonl(records, "arar_collection_9.jsonl")
